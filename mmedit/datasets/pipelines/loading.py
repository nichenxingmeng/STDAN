...
@PIPELINES.register_module()
class LoadImageFromFileList_saliency(LoadImageFromFile):
    """Load images from a file list together with their saliency maps.

    It accepts a list of paths, reads each frame, loads the matching
    saliency map (grayscale) and concatenates it as an extra channel, so
    the returned image is ``HxWx(C+1)``. The saliency channel is later
    used by the saliency-weighted loss.

    The saliency map path is derived from the image path by taking the
    part of the path *after* ``path_split_token`` and joining it onto
    ``saliency_folder``. For example, with the default token ``'HR'`` an
    image ``.../HR/0001/0000.png`` maps to
    ``{saliency_folder}/0001/0000.png``.

    Args:
        saliency_folder (str): Root folder that stores the saliency maps.
            Required.
        path_split_token (str): Substring used to split the image path;
            the portion after it (relative path) is appended to
            ``saliency_folder``. Default: 'HR'.
        io_backend (str): io backend where images are store. Default: 'disk'.
        key (str): Keys in results to find corresponding path. Default: 'gt'.
        flag (str): Loading flag for images. Default: 'color'.
        channel_order (str): Order of channel, candidates are 'bgr' and 'rgb'.
    ...
    def __init__(self,
        ...
        if saliency_folder is None:
            raise ValueError(
                '`saliency_folder` must be provided for '
                'LoadImageFromFileList_saliency.')
        # normalize to a trailing separator so os.path.join behaves
        self.saliency_folder = str(saliency_folder)
    ...
    def __call__(self, results):
        ...
        filepaths = [str(v).replace('00000', '0') for v in filepaths]
        ...
        for filepath in filepaths:
            ...
            saliencypath = osp.join(self.saliency_folder, rel_path)
            img_bytes = self.file_client.get(filepath)
            img = mmcv.imfrombytes(
                img_bytes, flag=self.flag,
                channel_order=self.channel_order)  # HWC
            saliency = mmcv.imread(saliencypath, flag='grayscale') 
            ...
            img = np.concatenate((img, saliency), axis=2)
            imgs.append(img)
...
@PIPELINES.register_module()
class LoadImageFromFileList_ope(LoadImageFromFile):
    """Load images from a file list together with an omni-positional
    encoding (OPE) map.

    Reads each frame and concatenates a matching single-channel OPE map as
    an extra channel, producing an ``HxWx(C+1)`` image. The STDAN backbone
    reads this extra channel as the ``condition = lqs[:, :, 3]`` positional
    cue for the OPE-guided spatial DCN alignment (STCA).

    The OPE maps are expected to be precomputed offline and stored the same
    way as the frames (grayscale PNG mirroring the frame folder layout).
    The map path is derived from the image path by taking the part after
    ``path_split_token`` and joining it onto ``ope_folder``: with the
    default token ``'LR'`` an image ``.../LR/0001/0000.png`` maps to
    ``{ope_folder}/0001/0000.png``.

    Args:
        ope_folder (str): Root folder that stores the OPE maps. Required.
        path_split_token (str): Substring used to split the image path; the
            portion after it (relative path) is appended to ``ope_folder``.
            Default: 'LR'.
        io_backend (str): io backend where images are store. Default: 'disk'.
        key (str): Keys in results to find corresponding path. Default: 'lq'.
        flag (str): Loading flag for images. Default: 'color'.
        channel_order (str): Order of channel, candidates are 'bgr' and
            'rgb'. Default: 'bgr'.
        convert_to (str | None): The color space of the output image. If
            None, no conversion is conducted. Default: None.
        save_original_img (bool): If True, maintain a copy of the image in
            `results` dict with name of `f'ori_{key}'`. Default: False.
        kwargs (dict): Args for file client.
    """
    ...
    def __call__(self, results):
        ...
        for filepath in filepaths:
            ...
            img = mmcv.imfrombytes(
                img_bytes, flag=self.flag,
                channel_order=self.channel_order)  # HWC
            ope = mmcv.imread(opepath, flag='grayscale')
            ...
            ope = np.expand_dims(ope, axis=2)
            img = np.concatenate((img, ope), axis=2)
            imgs.append(img)
            shapes.append(img.shape)
            if self.save_original_img:
                ori_imgs.append(img.copy())
